...
class DeplacementController extends Controller
{
    public function index(Request $request)
    {
        ...
        $request['where'] = ['column' => 'mouvement_type_id', 'value' => 1];
    ...
    public function generatePdf($id)
    {
        ...
        $html = view('pdf.deplacement', $datas)->render();
        $mpdf = new \Mpdf\Mpdf([
            'mode' => 'utf-8',
    ...
    public function inventoryPdf($id)
    {
        ...
        foreach ($afterInventory->first()->inventoryPvas as $inventoryPva) {
            $variations = $inventoryPva->productVariationAttribute->variationAttribute->childVariationAttributes->map(function ($childVa) {
                return $childVa->attribute->title;
            });
        ...
        foreach ($afterInventory->last()->inventoryPvas as $inventoryPva) {
        ...
        $html = view('pdf.deplacementInventory', $datas)->render();
    ...
    public function store(Request $requests)
    {
        ...
        $deplacements = collect($requests->except('_method'))->map(function ($request) {
            $request["account_user_id"] = getAccountUser()->id;
            $account_id = getAccountUser()->account_id;
            $request['code'] = DefaultCodeController::getAccountCode('Deplacement', $account_id);
            $request['mouvement_type_id'] = 1;
            $deplacement_only = collect($request)->only('code', 'mouvement_type_id', 'from_warehouse', 'to_warehouse', 'description', 'statut', 'account_user_id');
            $deplacement = Mouvement::create($deplacement_only->all());
            if (isset($request['productVariationAttributes'])) {
                if ($deplacement->statut == 1) {
                    $inventoryAfter[] = [
                        "mouvement_id" => $deplacement->id,
                        "inventory_type_id" => 2,
                        "warehouse_id" => $deplacement->from_warehouse,
                        "productVariationAttributes" => collect($request['productVariationAttributes'])->pluck('id')->toArray()
                    ];
                    $inventoryAfter[] = [
                        "mouvement_id" => $deplacement->id,
                        "inventory_type_id" => 2,
                        "warehouse_id" => $deplacement->to_warehouse,
                        "productVariationAttributes" => collect($request['productVariationAttributes'])->pluck('id')->toArray()
                    ];
                ...
                foreach ($request['productVariationAttributes'] as $pvaData) {
                    ...
                    if ($deplacement->statut == 1) {
                        WarehouseController::mouve($newSop, $deplacement);
                    }
                }
                if ($deplacement->statut == 1) {
                    $inventoryBefore[] = [
    ...
    public function edit(Request $request, $id)
    {
        ...
        if (isset($request['products']['active'])) {
            ...
            $mouvement = Mouvement::find($id);
            $mouvementPvaDatas = [];
            $mouvement->mouvementPvas->map(function ($mouvementPva) use (&$mouvementPvaDatas) {
                ...
                $mouvementPvaDatas[$mouvementPva->productVariationAttribute->product->id]['productVariations'][$mouvementPva->id] = [
                    "id" => $mouvementPva->productVariationAttribute->id,
    ...
    public function update(Request $requests)
    {
        $validator = Validator::make($requests->except('_method'), [
            ...
            '*.productVariationAttributes.*.id' => [
                ...
                function ($attribute, $value, $fail) use ($requests) {
                    $keys = explode('.', $attribute); // Sépare la clé en segments
                    $firstIndex = $keys[0];
                    $deplacemnt = Mouvement::find($requests[$firstIndex]['id']);
                    if (!$deplacemnt->from_warehouse) {
                        $fail("Warehouse ID not found");
                    }
                    $warehousePva = WarehousePva::where(['product_variation_attribute_id' => $value, 'warehouse_id' => $deplacemnt->from_warehouse])->first();
                    if (!$warehousePva) {
                        $fail("Warehouse product not found");
                    }
                },
        ...
        $deplacements = collect($requests->except('_method'))->map(function ($request) {
            $deplacement_only = collect($request)->only('id', 'to_warehouse', 'from_warehouse', 'description', 'statut');
            $deplacement = Mouvement::find($deplacement_only['id']);
            $deplacement->update($deplacement_only->all());
            if (isset($request['productVariationAttributes'])) {
                if ($deplacement->statut == 1) {
                    $inventoryAfter[] = [
                        "mouvement_id" => $deplacement->id,
                ...
                $deplacementProducts = MouvementPva::where('mouvement_id', $request['id'])->get();
                $deplacementProductIds = collect($request['productVariationAttributes'])->pluck('id')->toArray();
                //récupérer les enregistrements a supprimée
                $mouvementPvaToDeletes = $deplacementProducts->map(function ($deplacementProduct) use ($deplacementProductIds) {
                    if (!in_array($deplacementProduct->product_variation_attribute_id, $deplacementProductIds))
                        return $deplacementProduct->id;
                })->filter();
                //supprimer les produits manquant
                $mouvementPvaToDeletes->map(function ($mouvementPvaToDelete) {
                    $deplacementProduct = MouvementPva::find($mouvementPvaToDelete);
                    $deplacementProduct->delete();
                });
                foreach ($request['productVariationAttributes'] as $pvaData) {
                    $isExist = MouvementPva::where(['product_variation_attribute_id' => $pvaData['id'], 'mouvement_id' => $deplacement->id, 'statut' => 1])->first();
                    if ($isExist) {
                        $isExist->update([
                        ...
                        if ($deplacement->statut == 1) {
                            WarehouseController::mouve($isExist, $deplacement);
                        }
                    } else {
                        $newSop = MouvementPva::create([
                            'product_variation_attribute_id' => $pvaData['id'],
            ...
            $deplacement = Mouvement::find($deplacement->id);
            return $deplacement;
        });
        return response()->json([
            'statut' => 1,
