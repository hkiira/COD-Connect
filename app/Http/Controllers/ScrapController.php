...
use App\Helpers\ScrapingHelper;

class ScrapController extends Controller
{
    public static $url = 'https://api.asapdelivery.ma';
    public static $scrap_url = 'https://app.asapdelivery.ma';
    ...
    public function returnOrders($returnId)
    {
        ...
        $response = $client->get('/exportbls.php?id=' . $returnId, [
            'headers' => [
                'Cookie' => $sessionId,
            ]
        ]);
    ...
    public function createOrder($dataOrder, $sessionId)
    {
        ...
        $response = $client->post('/inc/ramassage.php', [
            'form_params' => $dataOrder,
            'headers' => [
                'Cookie' => $sessionId,
            ]
        ]);

        $uploadResponse = $response->getBody()->getContents();
        ...
        foreach ($rows as $row) {
            ...
            if ($cells->length >= 7) {
                ...
                $data[] = [
                    ...
                    'date_creation' => trim($cells->item(6)->textContent),
    ...
    public function instanceOrders($statusId)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($cells->length >= 12) {
                ...
                $data[] = [
                    'id' => $id,
                    'asap_code' => trim($cells->item(9)->textContent),
                    'space_code' => trim($cells->item(10)->textContent),
                ];
            }
        }
        return $data;
    }
    public function getLastStatuses($keyword)
    {
        $sessionId = $this->login();
        $client = new Client([
            'base_uri' => self::$scrap_url,
